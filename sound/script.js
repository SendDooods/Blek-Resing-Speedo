document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkAudioElements = () => {
        ...
        Object.keys(els.audio).forEach(key => {
            const audio = els.audio[key];
            console.log(`${key}:`, audio ? 'Found' : 'NOT FOUND');
        });
    };

    // Make debug function available globally
    window.checkAudioElements = checkAudioElements;

    // Test function to manually play sounds
    window.testAudio = (soundName) => {
        console.log('Testing audio:', soundName);
        const audio = els.audio[soundName];
        if (audio) {
            playOnceAudio(audio);
        } else {
            console.log('Audio not found:', soundName);
        }
    };
